...
const QuotaQuery = () => {
  ...
  const query = async () => {
    if (!value.trim()) return showError('请输入要查询的用户名或用户 ID');
    setQuerying(true);
    try {
    ...
    <div className='mt-[60px] px-2 max-w-3xl mx-auto'>
      <Card>
        <div className='flex flex-col items-center gap-4 py-6'>
          ...
          <Typography.Text type='tertiary'>按用户名或用户 ID 精确查询当前余额。每次收费 {renderQuota(status.fee)}，费用进入低保池。</Typography.Text>
          <Typography.Text>今日已查询 {status.used_today} 次{status.daily_limit > 0 ? `，剩余 ${status.remaining_today} 次` : '，不限次数'}</Typography.Text>
          <Form className='w-full max-w-lg'>
            <Form.Slot label='查询方式'><Radio.Group value={queryType} onChange={(e) => setQueryType(e.target.value)}><Radio value='username'>用户名</Radio><Radio value='id'>用户 ID</Radio></Radio.Group></Form.Slot>
            <Form.Input label={queryType === 'id' ? '用户 ID' : '用户名'} value={value} onChange={setValue} placeholder={queryType === 'id' ? '请输入完整用户 ID' : '请输入完整用户名'} />
          </Form>
