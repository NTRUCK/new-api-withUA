...
const EditChannelModal = (props) => {
  ...
  const handleConvertToMultiKey = async () => {
    Modal.confirm({
      title: t('转为多 Key 渠道'),
      content: t(
        '此操作会把当前渠道转换为多 Key 模式，原有密钥会作为第 1 个 Key 保留。转换后不能在网页端切回单 Key，是否继续？',
      ),
      centered: true,
      onOk: async () => {
        try {
          const res = await API.post(
            `/api/channel/${channelId}/convert_multi_key`,
          );
          if (res?.data?.success) {
            showSuccess(t('转换成功'));
            await loadChannel();
  ...
  const showConvertToMultiKeyButton =
            ...
            const advancedSettingsContent = (
              <div className='space-y-4'>
                ...
                <div className='pt-3'>
                  ...
                  />
                  <Form.Switch
                    field='hide_upstream_info'
                    label={t('隐藏上游信息')}
                    checkedText={t('开')}
                    uncheckedText={t('关')}
                    onChange={(value) =>
                      handleChannelSettingsChange('hide_upstream_info', value)
                    }
                    extraText={t(
                      '将响应中的上游模型名替换为用户请求的公开模型名，并移除可识别上游的元数据',
                    )}
                  />
                  <Form.Switch
