...
export const channelFormSchema = z
  .object({
    ...
    empty_response_no_billing: z.boolean().optional(),
...
export const CHANNEL_FORM_DEFAULT_VALUES: ChannelFormValues = {
  ...
  empty_response_no_billing: false,
  ...
  if (channel.setting) {
    try {
      ...
      extraSettings = {
        ...
        empty_response_no_billing: parsed.empty_response_no_billing || false,
      }
    } catch (error) {
...
export function buildSettingJSON(formData: ChannelFormValues): string {
  const settingObj: Record<string, unknown> = {
    ...
    empty_response_no_billing: formData.empty_response_no_billing || false,
  }
