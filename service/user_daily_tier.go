...
type memoryUserDailyCounter struct {
	...
	count   map[int]int64    // userId -> 已用次数
}
...
func userDailyTierKey(userId int) string {
	return fmt.Sprintf("%s:%s:%d", userDailyTierRedisPrefix, userDailyTierPeriod(), userId)
}

// GetUserDailyRequestCount 返回用户当日已成功请求次数
func GetUserDailyRequestCount(userId int) int64 {
	if common.RedisEnabled {
		ctx := context.Background()
		val, err := common.RDB.Get(ctx, userDailyTierKey(userId)).Int64()
		if err != nil {
			return 0
		}
		return val
	...
	return memUserDailyCounter.count[userId]
}

// IncrUserDailyRequestCount 在一次成功调用后将用户当日计数加一
func IncrUserDailyRequestCount(userId int) {
	if common.RedisEnabled {
		ctx := context.Background()
		key := userDailyTierKey(userId)
	...
	if memUserDailyCounter.period != period {
		...
		memUserDailyCounter.count = make(map[int]int64)
	}
	memUserDailyCounter.count[userId]++
}

// CheckUserDailyTierLimit 校验用户是否达到每日硬限额
// 返回 (allowed, used, hardLimit)；未启用或豁免时 allowed=true
func CheckUserDailyTierLimit(userId int, isAdmin bool) (allowed bool, used int64, hardLimit int) {
	...
	used = GetUserDailyRequestCount(userId)
...
func GetUserDailyTierMultiplier(userId int, isAdmin bool) float64 {
	if !setting.UserDailyTierEnabled {
		return 1.0
	}
	if isAdmin && setting.UserDailyTierAdminExempt {
...
func IncrUserDailyTierUsage(userId int, isAdmin bool) {
	if !setting.UserDailyTierEnabled {
		return
	}
	if isAdmin && setting.UserDailyTierAdminExempt {
	...
	IncrUserDailyRequestCount(userId)
}
