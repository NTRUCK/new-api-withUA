...
type User struct {
	...
	DiscordUsername   string         `json:"-" gorm:"column:discord_username"`
	DiscordGlobalName string         `json:"-" gorm:"column:discord_global_name"`
	DiscordAvatar     string         `json:"-" gorm:"column:discord_avatar"`
...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int) ([]*User, int64, error) {
	...
	// 构建搜索条件
	likeCondition := "username LIKE ? OR email LIKE ? OR display_name LIKE ? OR discord_id = ?"
	likeArgs := []interface{}{"%" + keyword + "%", "%" + keyword + "%", "%" + keyword + "%", keyword}

	// 尝试将关键字转换为整数ID
	keywordInt, err := strconv.Atoi(keyword)
	if err == nil {
		// 如果是数字，同时搜索ID、Discord ID和其他字段
...
type BatchDisableViolation struct {
	ReasonCode   string
	ReasonText   string
	ListPublicly bool
	OperatorId   int
...
func BatchDisableUsersWithViolation(userIds []int, violation *BatchDisableViolation) (disabledIds []int, listedCount int, err error) {
	if len(userIds) == 0 {
		return disabledIds, 0, nil
	}
	var eligibleIds []int
	err = DB.Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&User{}).Where("id IN ? AND id <> ? AND role < ?", userIds, 1, common.RoleAdminUser).Pluck("id", &eligibleIds).Error; err != nil {
			return err
		}
		for _, userId := range eligibleIds {
			result := tx.Model(&User{}).Where("id = ? AND status = ?", userId, common.UserStatusEnabled).Update("status", common.UserStatusDisabled)
			if result.Error != nil {
				return result.Error
			}
	...
	return disabledIds, listedCount, nil
}
